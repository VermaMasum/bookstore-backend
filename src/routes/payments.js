...
router.post('/', async (req, res, next) => {
  try {
    const { orderType, vendorOrderId, schoolOrderId, companyId, schoolId, amount, method, notes, paymentDate } = req.body;
    ...
    if (schoolOrderId) {
      ...
      if (order && ['PENDING', 'PARTIAL'].includes(order.status)) {
        ...
        if (totalPaid >= orderTotal) {
          await prisma.schoolOrder.update({ where: { id: parseInt(schoolOrderId) }, data: { status: 'DELIVERED' } });
        }
      }
    }
