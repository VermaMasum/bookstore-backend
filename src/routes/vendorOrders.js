  ...
  try {
    ...
    for (const item of order.items) {
      ...
      if (!inv || inv.quantity < item.quantity)
        return res.status(400).json({ success: false, message: `Insufficient stock for "${item.book.title}"` });
    }
    ...
    await prisma.$transaction([
      ...
      prisma.vendorOrder.update({ where: { id: orderId }, data: { status: 'DISPATCHED' } }),
    ]);
